...
def user_interaction():
    ...
    while True:
        ...
        try:
            ...
            print("Ошибка ввода: используйте символы UTF-8 "
                  "(например, латиницу).")
            continue
        ...
        if choice == '1':
            try:
                ...
                print(f"После cast_to_object_list: "
                      f"количество объектов={len(vacancy_objects)}")
                for i, vacancy in enumerate(vacancy_objects):
                    print(f"Перед сохранением вакансии #{i}: "
                          f"name={vacancy.name}, url={vacancy._url}, "
                          f"type(url)={type(vacancy._url)}, "
                          f"salary={vacancy.salary}")
                    json_saver.add_data(vacancy.to_dict())
                    print(f"После сохранения вакансии #{i}: "
                          f"name={vacancy.name}, url={vacancy._url}")
                print(f"Найдено {len(vacancy_objects)} вакансий. "
                      f"Сохранено в файл.")
            except UnicodeDecodeError:
                print("Ошибка ввода: используйте символы UTF-8 "
                      "(например, латиницу).")
            ...
            try:
                filter_words = input("Введите ключевые слова через пробел: "
                                     ).split()
                vacancies = json_saver.get_data()
                vacancy_objects = Vacancy.cast_to_object_list(vacancies)
                filtered_vacancies = filter_vacancies(vacancy_objects,
                                                      filter_words)
            ...
            try:
                salary_range = input(
                    "Введите диапазон зарплат (например, 100000-150000 или "
                    "100000): ").strip()
                if '-' in salary_range:
